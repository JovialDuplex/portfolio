  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {statsList.map((stat, index)=>(
        <Card key={index} className="bg-(--bg-card) w-full border border-(--border-card) hover:border-blue-600/40 hover:shadow-lg hover:shadow-blue-900/10 transition-all duration-200 rounded-(--radius-card)">
          <CardContent className="pt-5 pb-5 px-5 flex flex-col gap-2">
            ...
            <p className="text-3xl sm:text-4xl font-extrabold text-center text-(--text-accent) tracking-tight leading-none my-2">{stat.value}</p>
          </CardContent>
        </Card>
      ))}
    </div>
